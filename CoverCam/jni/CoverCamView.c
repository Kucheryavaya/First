...
JNIEXPORT void JNICALL Java_com_vm_covercam_CoverCamView_decode
 (JNIEnv * pEnv, jclass pClass, jobject pTarget, jbyteArray pSource, jint pCheck){
    ...
    for (lY = 0, lYIndex = 0; lY < lBitmapInfo.height; ++lY) {
        ...
        for (lX = 0; lX < lBitmapInfo.width; ++lX, ++lYIndex) {
            ...
            if (pCheck == 1)
            {
            	lK = 0.299 * lColorR + 0.587 * lColorG + 0.114 * lColorB;
            	lR = lG = lB = lK;
            	lBitmapContent[lYIndex] = color(lR, lG, lB);
            }
            if (pCheck == 2)
            {
            	lBitmapContent[lYIndex] = color(0,lColorG,0);
            }
            if (pCheck == 3)
            {
                lBitmapContent[lYIndex] = color(lColorR,0,0);
            }
            if (pCheck == 4)
            {
                lBitmapContent[lYIndex] = color(0,0,lColorB);
            }
